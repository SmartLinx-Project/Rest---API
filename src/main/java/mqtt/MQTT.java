...
public class MQTT {
    static final String MQTT_BROKER = "example.com";
    static final int MQTT_PORT = 8883;
    static final String keystorePath = "resources/example.jks";
    static final String keystorePassword = "example";
    ...
    public static MqttClient connect() throws MqttException {
        ...
        MqttClient client = new MqttClient("ssl://" + MQTT_BROKER + ":" + MQTT_PORT, clientId, null);

        MqttConnectOptions connOpts = new MqttConnectOptions();
        try {
            connOpts.setSocketFactory(SslUtil.getTruststoreFactory(keystorePath, keystorePassword));
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @return true if the device left the hub, false otherwise
     * @throws MqttException mqtt exception thrown because of failed mqtt request
     * @throws IllegalStateException if device is unreachable
     */
