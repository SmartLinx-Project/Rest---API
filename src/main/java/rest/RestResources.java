...
public class RestResources {
    ...
    @Path("/device")
    @POST
    public Response addDevice(Device newDevice, @QueryParam("token") String token, @QueryParam("uid") String uid) {
        String mail = Firebase.verifyToken(token, uid);
        if(mail == null)
            return Response.status(Response.Status.UNAUTHORIZED).build();

        try {
            if(DB.hasMemberRightsOnRoom(mail, newDevice.getRoomID()))
